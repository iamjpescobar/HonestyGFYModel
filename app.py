...
def get_weather(team_name):
    # API key: replace with your actual OpenWeatherMap key
    API_KEY = "your_openweathermap_api_key_here"
    # Basic logic to fetch weather based on city
    try:
        url = f"http://api.openweathermap.org/data/2.5/weather?q={team_name}&appid={API_KEY}&units=imperial"
        data = requests.get(url).json()
        temp = data['main']['temp']
        condition = data['weather'][0]['main']
        return f"🌡️ {temp}°F | {condition}"
    except:
        return "🌡️ Weather N/A"

# 2. MAIN INTERFACE (No Sidebar)
st.title("Los Cappers Lab 🧪")
games = get_todays_games()

if games:
    # Game Selector Header
    st.markdown("### 📅 Select Active Matchup")
    game_options = [f"{g['away']} @ {g['home']}" for g in games]
    
    # Using columns to create a "Front Page" selector
    cols = st.columns(len(game_options))
    for i, game in enumerate(games):
        if cols[i].button(f"{game['away']} @ {game['home']}"):
    ...
    if 'selected_game' in st.session_state:
        ...
        st.markdown("---")
        # Header with Weather
        st.header(f"Pro-Report: {g['away']} @ {g['home']}")
        st.subheader(f"{g['away_pitcher']} vs {g['home_pitcher']}")
        
        col1, col2 = st.columns(2)
        col1.markdown(f"**Venue Weather (Home Team):** {get_weather(g['home'].split()[-1])}")
...
if games:
    ...
    st.info("Awaiting live MLB schedule initialization data streams.")# --- NOW PASTE YOUR 300+ LINES OF LOGIC HERE ---
        # The logic below will now render directly under this header
        # instead of being trapped in a sidebar.
